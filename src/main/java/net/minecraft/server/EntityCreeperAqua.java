...
public class EntityCreeperAqua extends EntityMonster {
	...
	public EntityCreeperAqua(World world) {
        super(world);
        this.goalSelector.a(1, new PathfinderGoalFloat(this));
        this.goalSelector.a(2, new PathfinderGoalSwellAqua(this));
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        nbttagcompound.setByte("ExplosionBRadius", (byte) this.explosionRadius);
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("ExplosionBRadius", 99)) {
            this.explosionRadius = nbttagcompound.getByte("ExplosionBRadius");
        }
    ...
    public void die(DamageSource damagesource) {
        ...
        if (damagesource.getEntity() instanceof EntitySkeleton) {
            int i = Item.getId(Items.POTION_FALL);
            int j = Item.getId(Items.ARROWSPEED);
            ...
            this.record = k;
            // CraftBukkit end
        } else if (damagesource.getEntity() instanceof EntityCreeperAqua && damagesource.getEntity() != this && ((EntityCreeperAqua) damagesource.getEntity()).isPowered() && ((EntityCreeperAqua) damagesource.getEntity()).cp()) {
            ((EntityCreeperAqua) damagesource.getEntity()).cq();
            // CraftBukkit start
            // this.a(new ItemStack(Items.SKULL, 1, 4), 0.0F);
            headDrop = new ItemStack(Items.SKULL, 1, 6);
            // CraftBukkit end
        }
    ...
    public void onLightningStrike(EntityLightning entitylightning) {
        ...
        // CraftBukkit start
        if (CraftEventFactory.callCreeperAquaPowerEvent(this, entitylightning, org.bukkit.event.entity.CreeperAquaPowerEvent.PowerCause.LIGHTNING).isCancelled()) {
            return;
        }
    ...
    private void cr() {
        if (!this.world.isClientSide) {
            ...
            float f = this.isPowered() ? 3.5F : 1.75F;

            ExplosionPrimeEvent event = new ExplosionPrimeEvent(this.getBukkitEntity(), this.explosionRadius * f, false);
            this.world.getServer().getPluginManager().callEvent(event);
            if (!event.isCancelled()) {
                this.world.createExplosionB(this, this.locX, this.locY, this.locZ, event.getRadius(), event.getFire(), flag);
                this.die();
            } else {
                fuseTicks = 0;
            }
