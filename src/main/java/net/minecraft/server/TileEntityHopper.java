...
public class TileEntityHopper extends TileEntityContainer implements IHopper, IUpdatePlayerListBox {
    ...
    public static boolean isTitaneChest = false;
    ...
    public static IInventory b(World world, double d0, double d1, double d2) {
        ...
        if (tileentity instanceof IInventory) {
            ...
            if (object instanceof TileEntityIronChest) {
            	isIronChest = true;
                Block block = world.getType(new BlockPosition(i, j, k)).getBlock();

                if (block instanceof BlockIronChest) {
                    object = ((BlockIronChest) block).f(world, blockposition);
                }
            }
            if (object instanceof TileEntityTitaneChest) {
            	isTitaneChest = true;
                Block block = world.getType(new BlockPosition(i, j, k)).getBlock();

                if (block instanceof BlockTitaneChest) {
                    object = ((BlockTitaneChest) block).f(world, blockposition);
                }
            }
        }
