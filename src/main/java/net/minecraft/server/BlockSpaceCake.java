...
public class BlockSpaceCake extends Block {

    public static final BlockStateInteger BITES = BlockStateInteger.of("bites", 0, 6);

    protected BlockSpaceCake() {
        super(Material.CAKE);
        this.j(this.blockStateList.getBlockData().set(BlockSpaceCake.BITES, Integer.valueOf(0)));
        this.a(true);
    }

    public void updateShape(IBlockAccess iblockaccess, BlockPosition blockposition) {
        float f = 0.0625F;
        float f1 = (float) (1 + ((Integer) iblockaccess.getType(blockposition).get(BlockSpaceCake.BITES)).intValue() * 2) / 16.0F;
        float f2 = 0.5F;
    ...
    public AxisAlignedBB a(World world, BlockPosition blockposition, IBlockData iblockdata) {
        float f = 0.0625F;
        float f1 = (float) (1 + ((Integer) iblockdata.get(BlockSpaceCake.BITES)).intValue() * 2) / 16.0F;
        float f2 = 0.5F;
    ...
    private void b(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman) {
        if (entityhuman.j(false)) {
            ...
            // CraftBukkit end
            entityhuman.addEffect(new MobEffect(MobEffectList.INVISIBILITY.id, 10000, 0));
            entityhuman.addEffect(new MobEffect(MobEffectList.WEAKNESS.id, 3000, 0));
            entityhuman.addEffect(new MobEffect(MobEffectList.SLOWER_MOVEMENT.id, 4000, 0));
            entityhuman.addEffect(new MobEffect(MobEffectList.CONFUSION.id, 300, 0));
            int i = ((Integer) iblockdata.get(BlockSpaceCake.BITES)).intValue();

            if (i < 6) {
                world.setTypeAndData(blockposition, iblockdata.set(BlockSpaceCake.BITES, Integer.valueOf(i + 1)), 3);
            } else {
    ...
    public IBlockData fromLegacyData(int i) {
        return this.getBlockData().set(BlockSpaceCake.BITES, Integer.valueOf(i));
    }

    public int toLegacyData(IBlockData iblockdata) {
        return ((Integer) iblockdata.get(BlockSpaceCake.BITES)).intValue();
    }

    protected BlockStateList getStateList() {
        return new BlockStateList(this, new IBlockState[] { BlockSpaceCake.BITES});
    }

    public int l(World world, BlockPosition blockposition) {
        return (7 - ((Integer) world.getType(blockposition).get(BlockSpaceCake.BITES)).intValue()) * 2;
    }
