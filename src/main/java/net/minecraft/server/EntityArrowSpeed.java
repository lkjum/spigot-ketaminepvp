...
public class EntityArrowSpeed extends Entity implements IProjectile {
    ...
    public EntityArrowSpeed(World world) {
        super(world);
        this.j = 10.0D;
        this.setSize(0.5F, 0.5F);
    }

    public EntityArrowSpeed(World world, double d0, double d1, double d2) {
        super(world);
        this.j = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityArrowSpeed(World world, EntityLiving entityliving, EntityLiving entityliving1, float f, float f1) {
        super(world);
        this.j = 10.0D;
    ...
    public EntityArrowSpeed(World world, EntityLiving entityliving, float f) {
        super(world);
        this.j = 10.0D;
        this.shooter = entityliving;
        this.projectileSource = (LivingEntity) entityliving.getBukkitEntity(); // CraftBukkit
        ...
        if (this.inGround) {
            ...
            if (movingobjectposition != null) {
                ...
                if (movingobjectposition.entity != null) {
                    //
                	movingobjectposition.entity.addEffect(new MobEffect(MobEffectList.FASTER_MOVEMENT.id, 300, 1));
                	//
                    ...
                    if (this.shooter == null) {
                        damagesource = DamageSource.arrowSpeed(this, this);
                    } else {
                        damagesource = DamageSource.arrowSpeed(this, this.shooter);
                    }
            ...
            {
                this.world.addParticle(EnumParticle.SPELL_MOB, this.locX + this.motX * (double) jj / 4.0D, this.locY + this.motY * (double) jj / 4.0D, this.locZ + this.motZ * (double) jj / 4.0D, -this.motX, -this.motY + 0.2D, -this.motZ, new int[10]);
            }
            //
            if (this.isCritical()) {
                for (j = 0; j < 4; ++j) {
    ...
    public void d(EntityHuman entityhuman) {
        if (!this.world.isClientSide && this.inGround && this.shake <= 0) {
            ...
            if (this.fromPlayer == 1 && !entityhuman.inventory.pickup(new ItemStack(Items.ARROWSPEED, 1))) {
                flag = false;
            }
